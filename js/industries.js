...
const Industries = {
  ...
  taxRate(sys, planet) {
    ...
    if (window.Boosts) r += Boosts.mag("industryTax");    // Tax Ghost blackbox (mag is negative)
  ...
  _yield(sys, planet, ex, commodity, mult = 1) {
    ...
    const boxInd = window.Boosts ? (1 + Boosts.mag("industryYield")) : 1;
    const gross = Math.round(INDUSTRYCFG.baseYield * suit * Extractors.yieldMult(ex) * bon.rate * mult * flagInd * boxInd);
