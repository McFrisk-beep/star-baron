...
username.js → content.js → bgm.js → market.js → galaxy.js → items.js → fleet.js → economy.js →
...
`bgm.js` after `content.js` — shared playlist overlay + player.)
...
| `galaxy.js` | `Galaxy` — deterministic procedural galaxy (sectors/systems/planets, each planet has a `type`/`cat`/`commodity`), local news log + mechanical local events. `factionOf()`/`factionColor()` derive a system's allegiance from the categories its planets work (Syndic space is Syndicate space, §5.4) — the galaxy chart colours nodes by it. |
...
| `mining.js` | `Mining` — asteroid mining (docs/SPACE_INTERACTIVITY.md §3): park a miner-class hull at a belt POI for untaxed clock-math ore batches into positions + the system bay. NPC crews drain the same finite pool on the site's clock (`npcTaken`), so you race them; `state.beltPools` stores only your own take, keyed by the site's generation. Escort hulls can guard a claim (`guardUids` on the op, status `guarding`), and each batch is rolled against `Raiders`. Server-owned once `docs/sql/mining_rpcs.sql` is applied — `app._catchup_mining` banks batches, rolls raids and lands hulls inside `app_pull`, and `Mining.serverOwned()` latches off the echoed slice; guests and projects without that SQL keep the local resolver. |
| `security.js` | `Security` — how much law is present, per system and per region (docs/SPACE_INTERACTIVITY.md §5.3). `score(sysId)` = `SECURITYCFG.sectorBase` + sector capital + station modules (Customs House / Lane Buoy up, Free Port / Black Market down) + Senate edicts (`routeSafetyAdd`) + a running war, clamped 0–1; `band()` buckets it for the galaxy chart's region blobs, `factors()` is the derivation the tooltip shows, `raidMult()` is what `Raiders` scales corsair odds by. Pirate dens are deliberately excluded — §7.1 keeps them hidden until found. Nothing stored. |
| `raiders.js` | `Raiders` — NPC piracy (docs/SPACE_INTERACTIVITY.md §4): corsair pressure on a parked mining claim, derived from the seam's own richness (which already carries the sector) × a pirate den in-system × Senate lane patrols × Syndicate standing in the Sprawl. `rollClaim(op, cycle, poi, qty)` is a pure function of (op, cycle) so offline banking lands exactly the raids a watched tab would have; `tookManifest()` robs NPC hauls near a den. Anti-grief (§6.6) is enforced here: a raid takes the interrupted batch and nothing else — never banked ore, never a hull. |
