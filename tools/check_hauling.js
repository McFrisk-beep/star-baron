...
assert.strictEqual(q.customsRisk, 0, "no customs risk without illicit");
const illicit = COMMODITIES.find(c => c.cat === "illicit");
assert.ok(illicit, "illicit commodity exists");
ctx.Game.state.stationInv.navos.blocks[illicit.id] = 20;
ctx.Game.state.positions[illicit.id] = 20;
const qi = Shipments.quote("navos", other.id, { [illicit.id]: 20 }, []);
assert.ok(qi.illicit, "illicit flag on quote");
assert.ok(qi.customsRisk > 0, "customs risk exposed for illicit manifests");
delete ctx.Game.state.stationInv.navos.blocks[illicit.id];
delete ctx.Game.state.positions[illicit.id];
const dests = Shipments.destinations();
assert.ok(dests.includes("navos") && dests.includes(other.id), "destinations = asset stations");
assert.ok(!dests.includes("no_such_station"), "unknown stations excluded");
