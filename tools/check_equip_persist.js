...
const APPLY_ORDER = [
  ...
  // It copies save_hygiene's app_commit body (the tail at the time) and adds
  // the mining merge.
  "mining_rpcs.sql",
  // Applied last: it copies mining_rpcs' app_commit body (the current tail) and
  // adds the piracy merges, so it must stay at the end — a file pasted after it
  // would need to copy THIS body instead.
  "piracy_rpcs.sql",
];
